...
func main() {
	...
	//cherryPicker := gitutils.NewCherryPicker(logger)
	//
	...
	//
	//logger.Info("Looking at upstream commits", "since", dsSince)
	//
	...
	//
	//	cherryPickOut, err := cherryPicker.Run(ctx, cl.DownstreamRepoPath, hash)
	//	if err != nil {
	...
	//		}
	//
	//		logger.Info("Successfully created issue", "URL", issue.HTMLURL)
	//		logger.Info("Aborting cherry-pick")
	//
	//		abortOut, err := cherryPicker.Abort(ctx, cl.DownstreamRepoPath)
	//		if err != nil {
	//			logger.Error(err, "Could not abort cherry-picking", "output", abortOut)
	//			os.Exit(1)
	//		}
	//
	//		continue
